...
(function (FieldType) {
    ...
    FieldType[FieldType["otherField"] = 4] = "otherField";
})(FieldType || (FieldType = {}));
(function($) {
  ...
  ConditionsBuilder.prototype = {
    ...
    collectDataFromNode: function(element) {
      ...
      if(klass) {
        ...
        // removes text " ( type )"
        if (vl && type === ".venkifield" && vl.indexOf(" (") > -1) vl = vl.substring(0,vl.indexOf(" ("));
  ...
  function onFieldSelectChanged(operatorSelect, ruleData) {
    ...
    return function(e) {
      ...
      for(var i=0; i < operators.length; i++) {
        ...
        if (operator.options)
            option.data("options", operator.options)
    ...
    switch(option.data("fieldType")) {
      ...
      case "text":
        if (option.val() !=="matchesRegex"){
            ...
            field.hide();
            changeFLink.show();
            changeFLink.attr("id","literal")
            changeFLink.after($("<input>", {"type": "text", "class": "value literal"}).show());
        }
      ...
      case "select":
        field.remove();
        changeFLink.hide();
        var select = $("<select>", {"class": "value venkifield select2-selection"})
        var options = fieldSelect.find("> :selected").data("options") || option.data("options");
        if (options){
            for(var i=0; i < options.length; i++) {
                var opt = options[i];
                select.append($("<option>", {"text": opt.label || opt.text, "value": opt.value}));
            }
        }
